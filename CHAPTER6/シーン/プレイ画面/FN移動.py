...
class 移動クラス:
    ...
    def 実行(self):
        ...
        elif main.DB.ゲーム.プレイ時間 % main.DB.ゲーム.難易度間隔 == 0:
        #　├┐（レベルの変わり目の場合）
            #↓
            #○ボスシーンを『ボス警告』に進行する
            #○ボス警告カウントを開始する
            #○プレイ時間を1つ進める ※ボスシーンを繰り返さない為
            main.DB.ゲーム.ボスシーン = シーンID.ボス警告
        ...
        #　└┐（その他）
            ...
            #○難易度をセットする
            main.DB.ゲーム.プレイ時間 += 1
            main.DB.ゲーム.難易度 = main.DB.ゲーム.プレイ時間 // main.DB.ゲーム.難易度間隔 + 1
        #┴　┴
    ...
    def Fnボス処理(self):
        #┬
        #○ボスシーンを確認する
        if main.DB.ゲーム.ボスシーン == None: return
        #│＼（僕警告ではない場合）
        #│ ↓
        #│ ▼処理を中断する
        #│
        #◇┐ボスシーンを進行する
        if main.DB.ゲーム.ボスシーン == シーンID.ボス警告:
        #　├┐（ボス警告の場合）
            #↓
            #○警告時間を減らす
            ...
            #┴
        elif main.DB.ゲーム.ボスシーン == シーンID.ボス対決:
        #　├┐（ボス対決の場合）
            #↓
            #◇┐ボスシーン(ボス登場)を進行する
