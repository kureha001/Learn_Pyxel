...
class 出現クラス:
    ...
    def 実行(self):
        ...
        if self.GAME.情報.プレイ時間 < 60 :return
        #│＼（対象外の場合）
        #│ ↓
        #│ ▼繰処理を中断する
        #│
    ...
    def Fn敵機(self):
		#┬
        #◇┐出現タイミングを確認する
        間隔_敵機 = max(60 - self.GAME.情報.難易度 * 10, 30)
        if self.GAME.情報.プレイ時間 % 間隔_敵機 != 0: return
        #　＼（対象外の場合）
        #　 ↓
        #　 ▼処理を中断する
        #│
        ...
        難易度  = self.GAME.情報.難易度
        敵機出現(self.GAME, X座標, -8, 種類ID, 難易度)
        #┴
	#────────────────────────────────────
    def Fn機雷(self):
		#┬
        #◇┐出現タイミングを確認する
        if self.GAME.情報.プレイ時間 % 仕様.機雷 != 0: return
        #　＼（対象外の場合）
        #　 ↓
        #　 ▼処理を中断する
        #│
    ...
    def Fnアイテム(self, 引数_間隔, 引数_種類ID):
		#┬
        #◇┐出現タイミングを確認する
        if self.GAME.情報.プレイ時間 % 引数_間隔 != 0: return
        #　＼（対象外の場合）
        #　 ↓
        #　 ▼処理を中断する
        #│
