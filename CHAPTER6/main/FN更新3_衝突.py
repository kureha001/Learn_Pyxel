...
class 衝突クラス:
    ...
    def Fn攻撃(self):
        ...
        for tmp自弾 in main.DB.obj弾_自機.copy():
			...
			#│ ▼繰り返し処理を抜ける
			#│
            #○処理方法を『弾(自機機による攻撃』で用意する
            引数 = (True, tmp自弾)
			#│
            #●敵機を攻撃する
            if self.Fn衝突判定( *引数, main.DB.obj敵機 ): continue
            #│＼（この弾が消失した場合）
            #│ ↓
            #│ ▼処理を中断する
			#│
    ...
    def Fn体当たり(self):
        ...
        for tmp自機 in main.DB.obj自機:
            ...
            #│ ▼繰処理を中断する
            #│
            #○処理方法を『自機による体当たり』で用意する
            引数 = (False, tmp自機)
            #│
            #●アイテムに体当たりする
            if self.Fn衝突判定( *引数, main.DB.objアイテム ): return
            #│＼（自機が消失した場合）
            #│ ↓
            #│ ▼処理を中断する
            #┴
        ...
        for tmp自機 in main.DB.obj自機:
            ...
            #│
            #●敵機に体当たりする
            if self.Fn衝突判定( *引数, main.DB.obj敵機 ): return
            #│＼（自機が消失した場合）
            #│ ↓
            #│ ▼処理を中断する
            #│
    ...
    def Fn衝突判定(self ,
        ...
        for tmp相手 in arg相手.copy():
			...
			#│
            #○相手の衝突条件を用意する
            相手 = (tmp相手.情報.X, tmp相手.情報.Y)
            相手 = (相手, tmp相手.FN衝突.仕様.衝突範囲)
			#│
            #●自分と相手の衝突を判定する
            if not 共通.Fn衝突判定( 自分, 相手 ): continue
            #│＼（衝突していない場合）
            #│ ↓
            #│ ▼次の相手を処理する
            #│
    ...
    def Fn衝突_攻撃(self,
        ...
        #│
        #○自分の威力を用意する
        仕様 = main.DB.obj自機共通.仕様
        威力 = (仕様.威力_弾) if arg手段区分 else (仕様.威力_体当たり)
        #│
        #●相手にダメージ(弾攻撃)を与える
        arg相手.FN衝突.実行( 威力 )
        #┴
