...
class 衝突クラス:
    ...
    def Fnセット_攻撃(self         ,
        ...
        for tmp弾 in main.DB.obj弾_自機.copy():
			...
			#│
            #◎└┐対象に体当たりする
            for tmp対象リスト in 引数_対象セット:
                #│＼（すべての対象を照会し終えた場合）
                #│ ↓
    			#│ ▼繰り返し処理を終える
                #●敵機を攻撃する
                if self.Fn判定_攻撃(tmp弾, tmp対象リスト): break
                #│＼（弾が破壊された場合）
                #│ ↓
    			#│ ▼次の弾を照会する
        #┴　┴　┴
    ...
    def Fnセット_体当たり(self     ,
        ...
        for tmp自機 in main.DB.obj自機:
            ...
            for tmp対象リスト in 引数_対象セット:
                ...
                #│
                #●アイテムに体当たりする
                if self.Fn判定_体当たり(tmp自機, tmp対象リスト): break
                #│＼（自機が破壊された場合）
                #│ ↓
    			#│ ▼次の自機を照会する
        #┴　┴　┴
    ...
    def Fn判定_攻撃(self    ,   #【戻り値】 True:破壊された／False：存在する
        ...
        for tmp対象 in 引数_対象リスト.copy():
            ...
            #　 ＼（相手が機雷の場合）
                ...
                #●敵機・弾(自機)・弾(敵機)を全滅する
                #▼弾が『破壊された』を返す
                self.Fn機雷_起爆(main.DB.obj敵機    )
                self.Fn機雷_起爆(main.DB.obj弾_自機 )
                self.Fn機雷_起爆(main.DB.obj弾_敵機 )
                return True
            #│
            #●弾(自機)での攻撃を処理する
