...
def Pアクション_ボス():
    #┬
    #〇出現タイミングを確認する
    タイミング判定 = (main.DB.情報.ボスシーン == シーンID.ボス登場)
    ...
    #│
    #○ボスシーンを『ボス対戦』に進行する
    main.DB.情報.ボスシーン = シーンID.ボス対決
    #│
    #○難易度に応じたボスキャラをリストアップする
    一覧 = 敵機.ボス出現DB.get(main.DB.情報.難易度, 敵機.ボス出現DB.get(0))
    ...
    #　├┐（反転タイミングの場合）
        ...
        for 種類ID in 一覧:
            ...
            出現数  += 1
            Y座標   = main.DB.仕様.キャラ幅.ボス * (出現数 + 1) * -2
