...
import  pyxel
import  処理._情報
from    ..敵機  import アイテムID

#┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#┃ＩＤ
#┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
class ID(): # FN衝突/FN発射 でも利用
...
class 特殊クラス():
    ...
    def Fn発動_速度(self,
        ...
        #●効果を発動する ※発動値：ダミー値
        値 = self.Fn発動済を削除(ID.追加速度)
        値 = 0.5 if 値 is not None else max(値 + 0.5 , 3)
        self.情報.発動[ID.追加速度] = (引数_持続時間, 値)
        #┴
    ...
    def Fn発動_連射(self,
        ...
        #●同類(連射)の発動済を削除する
        #●効果を発動する ※発動値：ダミー値
        値 = self.Fn発動済を削除(ID.連射)
        self.情報.発動[ID.連射] = (引数_持続時間, True)
        #┴
    ...
    def Fn発動_防御率(self,
        ...
        #│
        #●効果を発動する ※発動値：防御率
        self.情報.発動[ID.防御UP] = (引数_持続時間, 防御率)
        #┴　┴
    ...
    def Fn発動_貫通弾(self,
        ...
        #●同類(発射数)の発動済を削除する
        #●効果を発動する ※発動値：ダミー値
        値 = self.Fn発動済を削除(ID.貫通弾)
        self.情報.発動[ID.貫通弾] = (引数_持続時間, True)
        #┴
    ...
    def Fn発動_発射数(self,
        ...
        #●効果を発動する ※発動値：発射する弾数
        発射数 = self.Fn発動済を削除(ID.発射数)
        発射数 = 2 if 発射数 is not None else max((発射数 + 1),4)
        self.情報.発動[ID.発射数] = (引数_持続時間, 発射数)
        #┴
    ...
    def Fn発動_発射方向(self,
        ...
        for tmpID in range(アイテムID.発射_上下方向,アイテムID.発射_8方向):
            ...
            #　├┐（照会中のアイテムIDと一致した場合）
                ...
                #●効果を発動する ※発動値：発射する弾数
                #▼照会を中断する
                self.情報.発動[ID.発射方向] = (引数_持続時間 , 発射数)
                break
            #　└┐（その他）
        #┴　┴　┴
