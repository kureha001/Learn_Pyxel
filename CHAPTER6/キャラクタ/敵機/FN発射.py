...
class 発射クラス:
    ...
    def 実行(self):
		#┬
        #◇┐ボスの狙い撃ちを発射する  
        if self.個体.仕様.ボス区分 and not self.Fn発射禁止(15):
        #　├┐（ボスで発射タイミングの場合）
            #↓
            #●単発で発射する
            self.Fn発射_単発()
    ...
    def Fn発射禁止(self, 引数_間隔): #① 発射間隔
        ...
        #│
        #○発射タイミングを確認する
        if self.個体.情報.経過時間 % 引数_間隔 != 0: return True
        #│＼（発射タイミングではない場合）
        #│ ↓
        #│ ▼『発射不可能』を返す
        #│
    ...
    def Fn発射_単発(self):
        ...
        #│
        #○基本情報を用意する
        x1 = self.個体.情報.X + self.個体.仕様.オフセット
        y1 = self.個体.情報.Y + self.個体.仕様.オフセット
        #│
        #◎└┐自機に向けて発射する
